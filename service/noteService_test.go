...
func NewNoteRepositoryMockImpl() *NoteRepositoryMockImpl {
...
func NewNoteRepositoryMock() service.NoteServiceRepository {
	return &NoteRepositoryMockImpl{}
}
...
func TestNoteServiceImpl_SearchNotes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("service.NoteServiceImpl.SearchNotes() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("service.NoteServiceImpl.SearchNotes() = %v, want %v", got, tt.want)
			}
		})
	}
}
